...
const ProductsListPage = ({ history, match }) => {
  ...
  const { loading, products, errMessage, removeProductError , pageNumber , pages } = useSelector(
  ...
  useEffect(() => {
    if (userInfo && userInfo.isAdmin) {
      dispatch(fetchProductsAscync("", match.params.pagenumber))
    } else {
      history.push("/login")
    }
              ...
              <tr key={product._id}>
                ...
                <td>
                  <Button
                    ...
                    onClick={() => {
                      history.push(`/admin/updateproduct/${product._id}/edit`)
                    }}
                  >
